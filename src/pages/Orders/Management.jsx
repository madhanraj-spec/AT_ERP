...
export default function OrdersManagement() {
  ...
  const fetchDYRRDetail = async (id) => {
    setLoadingDetail(true);
    try {
      const { data: receipt } = await supabase.from('dyed_yarn_receipts').select('*').eq('id', id).single();
      const { data: items } = await supabase.from('dyed_yarn_receipt_items').select('*, location:master_locations(location_name)').eq('receipt_id', id);
      setViewDyrrData({ receipt, items });
    } catch (err) {
      console.error(err);
      ...
      <div style={{ border: '2px solid #7f1d1d', padding: '2rem', borderRadius: '8px' }}>
        ...
        <div className="grid-2-to-1" style={{ marginBottom: '2rem' }}>
          ...
          <div style={{ backgroundColor: '#f9fafb', padding: '1rem', borderRadius: '8px' }}>
            ...
            <p style={{ margin: '0.2rem 0' }}><strong>Approval Status:</strong> <span style={{ textTransform: 'uppercase', fontWeight: 700 }}>{getApprovalStatus(form.status)}</span></p>
            <p style={{ margin: '0.2rem 0' }}><strong>Yarn Status:</strong> <span style={{ textTransform: 'uppercase', fontWeight: 700 }}>{getYarnStatus(form.status).replace(/_/g, ' ')}</span></p>
          </div>
        </div>
...
function DYRRModal({ data, yarnCounts, onClose }) {
  const { receipt, items } = data;
  const formatYarn = (id) => {
    const y = yarnCounts.find(c => c.id === id);
    if (!y) return 'Unknown';
  ...
  return (
    <ModalWrapper title={`Dyed Yarn Received Receipt: ${receipt.dyrr_number}`} onClose={onClose}>
      <div style={{ border: '2px solid #16a34a', padding: '2rem', borderRadius: '8px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '2px solid #16a34a', paddingBottom: '1rem', marginBottom: '1.5rem' }}>
          <div>
            <h2 style={{ margin: 0, color: '#16a34a', fontWeight: '900' }}>DYED RECEIPT</h2>
            <p style={{ margin: 0, fontSize: '0.9rem', color: '#666' }}>ASHOK TEXTILES · Dyed Yarn Warehouse</p>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontWeight: '800' }}>#{receipt.dyrr_number}</div>
            <div style={{ fontSize: '0.8rem' }}>Date: {new Date(receipt.created_at || receipt.received_date).toLocaleDateString()}</div>
          </div>
        </div>
        ...
        <div className="grid-2-to-1" style={{ marginBottom: '2rem' }}>
          ...
          <div style={{ backgroundColor: '#f9fafb', padding: '1rem', borderRadius: '8px' }}>
            <div style={{ fontSize: '0.7rem', fontWeight: '800', textTransform: 'uppercase', color: '#999', marginBottom: '0.5rem' }}>Reference</div>
            <p style={{ margin: '0.2rem 0' }}><strong>DOF Number:</strong> {receipt.dof_number || '-'}</p>
            <p style={{ margin: '0.2rem 0' }}><strong>Source Type:</strong> {receipt.source_type === 'partner' ? 'Partner Receipt' : 'Production Return'}</p>
          </div>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
          <thead>
            <tr style={{ backgroundColor: '#16a34a', color: '#fff' }}>
              ...
              <th style={{ padding: '0.75rem', textAlign: 'left' }}>Store Location</th>
              <th style={{ padding: '0.75rem', textAlign: 'right' }}>Qty (kg)</th>
            </tr>
          </thead>
          <tbody>
            {items?.map((item, i) => (
              <tr key={i} style={{ borderBottom: '1px solid #eee' }}>
                <td style={{ padding: '0.75rem', fontWeight: '600' }}>{formatYarn(item.yarn_count_id)}</td>
                <td style={{ padding: '0.75rem' }}>{item.colour}</td>
                <td style={{ padding: '0.75rem', textTransform: 'capitalize' }}>{item.yarn_type || 'warp'}</td>
          ...
          <tfoot>
            <tr style={{ backgroundColor: '#f9fafb' }}>
              <td colSpan="5" style={{ padding: '0.75rem', textAlign: 'right', fontWeight: '800' }}>Total Received:</td>
