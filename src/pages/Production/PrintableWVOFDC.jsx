...
function PrintableWVOFDC({ wvof, order, producedQty }) {
  ...
  return (
    <div style={{ marginTop: '1.5rem', borderTop: '1px solid var(--border-current)', paddingTop: '1.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '1rem' }}>
        ...
        >
          <Printer size={16} /> Print / Download Delivery Challan
        </button>
      </div>
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '3px solid #800000', paddingBottom: '1rem', marginBottom: '1.5rem' }}>
            ...
            <div style={{ display: 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.7rem', color: '#800000', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Weaving Delivery Challan (WVOFDC)</div>
                </div>
              </div>
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ fontSize: '1rem', fontWeight: '900', color: '#800000', fontFamily: 'monospace' }}>{wvof.wvofdc_number || '—'}</div>
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          <div style={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: '800', color: '#800000', marginBottom: '8px', borderBottom: '1px solid rgba(128,0,0,0.2)', paddingBottom: '4px' }}>Weaving Process Details</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', fontSize: '12px' }}>
            <div>
              <label style={{ fontSize: '9px', textTransform: 'uppercase', color: '#888', fontWeight: '700', display: 'block' }}>WVOF Number</label>
              <div style={{ fontWeight: '700', fontFamily: 'monospace' }}>{wvof.weaving_number}</div>
            </div>
            <div>
              <label style={{ fontSize: '9px', textTransform: 'uppercase', color: '#888', fontWeight: '700', display: 'block' }}>Order Number</label>
              <div style={{ fontWeight: '700', fontFamily: 'monospace' }}>{order?.order_number || wvof.order?.order_number || '—'}</div>
            </div>
            <div>
              <label style={{ fontSize: '9px', textTransform: 'uppercase', color: '#888', fontWeight: '700', display: 'block' }}>Design Info</label>
              <div style={{ fontWeight: '700' }}>{order?.design_no || wvof.order?.design_no ? `${order?.design_no || wvof.order?.design_no} - ${order?.design_name || wvof.order?.design_name || ''}` : '—'}</div>
            </div>
            <div>
              <label style={{ fontSize: '9px', textTransform: 'uppercase', color: '#888', fontWeight: '700', display: 'block' }}>Weaving Type</label>
              <div style={{ fontWeight: '700' }}>{typeLabel}</div>
            </div>
            <div>
              <label style={{ fontSize: '9px', textTransform: 'uppercase', color: '#888', fontWeight: '700', display: 'block' }}>Loom / Partner</label>
              <div style={{ fontWeight: '700' }}>
                {wvof.weaving_type === 'in_house'
                  ? (wvof.machine_name || 'In-House Loom')
                  : `${wvof.partner_name || '—'} (${wvof.machine_name || '—'})`}
              </div>
            </div>
            <div>
        ...
        {/* Weft Yarn Returns */}
        <div style={{ marginBottom: '1.5rem' }}>
          <div style={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: '800', color: '#800000', marginBottom: '8px', borderBottom: '1px solid rgba(128,0,0,0.2)', paddingBottom: '4px' }}>Weft Yarn Return Details</div>
          {!wvof.yarn_returns || wvof.yarn_returns.length === 0 ? (
            <div style={{ fontSize: '11px', color: '#666', fontStyle: 'italic', padding: '8px 0' }}>No weft yarn returned.</div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
              <thead>
                <tr style={{ background: '#800000', color: 'white' }}>
                  ...
                  <th style={{ padding: '6px 8px', textAlign: 'right' }}>Qty Delivered (kg)</th>
                  <th style={{ padding: '6px 8px', textAlign: 'right' }}>Qty Returned (kg)</th>
                </tr>
              </thead>
              <tbody>
                {wvof.yarn_returns.map((r, idx) => (
        ...
        {/* Footer / Signatures */}
        <div style={{ marginTop: '3rem', display: 'flex', justifyStyle: 'space-between', justifyContent: 'space-between', fontSize: '10px', color: '#555' }}>
          <div>
            <div style={{ borderTop: '1px solid #333', width: '140px', paddingTop: '4px', textAlign: 'center' }}>Weaver Signature</div>
          </div>
          <div>
...
export default PrintableWVOFDC;
