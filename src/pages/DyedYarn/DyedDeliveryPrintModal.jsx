...
export default function DyedDeliveryPrintModal({ 
  delivery, 
  receipt, 
  data, 
  wof, 
  ...
  const dydr = delivery || receipt || data?.receipt || data;
  if (!dydr) return null;

  const rawItems = dydr.items || data?.items || [];
  const [partnerDetails, setPartnerDetails] = useState(dydr.partner || null);

  const handlePrint = () => {
    window.print();
  };

  const receiptNumber = dydr.dydr_number || dydr.receiptNumber || dydr.receipt_no || '—';
  ...
  const deliveryDate = formatDate(dydr.delivered_date || dydr.date || dydr.created_at);
  const deliveryTime = formatTime(dydr.created_at || dydr.delivered_date || dydr.date);

  const firstItem = rawItems[0] || {};
  const processType = firstItem.process_type || dydr.target_process || '';
  
  // Resolve Target Process Label & Form No
  let processLabel = 'Production Delivery';
  let formNumber = dydr.doc_no || dydr.wof_number || dydr.weaving_number || dydr.dof_number || '—';
  let machineName = dydr.machine_name || '—';
  let partnerLabel = partnerDetails?.partner_name || dydr.partner_name || dydr.partner?.partner_name || dydr.dyeing_unit?.partner_name || 'In-House';
  let isJobWork = false;

  if (processType === 'warping') {
    processLabel = 'Warping (WOF)';
    if (wof) {
      formNumber = wof.wof_number || formNumber;
      machineName = wof.machine_name || machineName;
      if (wof.wof_type === 'job_work') {
        isJobWork = true;
        partnerLabel = wof.partner_name || wof.partner?.partner_name || partnerLabel;
      } else {
        partnerLabel = 'In-House Warping';
      }
    }
  } else if (processType === 'weaving') {
    processLabel = 'Weaving (WVOF)';
    if (weaving) {
      formNumber = weaving.weaving_number || formNumber;
      machineName = weaving.machine_name || machineName;
      if (weaving.weaving_type === 'job_work') {
        isJobWork = true;
        partnerLabel = weaving.partner_name || weaving.partner?.partner_name || partnerLabel;
      } else {
        partnerLabel = 'In-House Weaving';
      }
    }
  } else if (processType === 'redyeing') {
    processLabel = 'Redyeing (DOF)';
    isJobWork = true;
  }

  // Fetch partner if not already present
  useEffect(() => {
    if (partnerDetails) return;
    const fetchPartner = async () => {
      let partnerId = dydr.partner_id || dydr.dyeing_unit_id;
      if (!partnerId && rawItems.length > 0) {
        const formId = rawItems[0]?.production_form_id;
      ...
      if (partnerId) {
        const { data: pData } = await supabase
          ...
          .eq('id', partnerId)
          .maybeSingle();
        if (pData) setPartnerDetails(pData);
      }
    };
    fetchPartner();
  }, [dydr, rawItems, processType, partnerDetails]);

  const formatCount = (item) => {
    if (item.master_yarn_counts) {
      return [item.master_yarn_counts.count_value, item.master_yarn_counts.spec, item.master_yarn_counts.spec1, item.master_yarn_counts.product_type].filter(Boolean).join(' ');
    }
    if (item.yarn_count) {
      if (typeof item.yarn_count === 'object') {
        return [item.yarn_count.count_value, item.yarn_count.spec, item.yarn_count.spec1, item.yarn_count.product_type].filter(Boolean).join(' ');
      }
      return String(item.yarn_count);
    }
    if (getFormatCount && item.yarn_count_id) {
      return getFormatCount(item.yarn_count_id);
    }
  ...
  const totalWeight = rawItems.reduce((sum, it) => sum + Number(it.weight ?? it.quantity_kg ?? 0), 0);
  const deliveredBy = dydr.delivered_by || dydr.logistics?.delivered_by || '—';
  const vehicleNo = dydr.vehicle_no || dydr.logistics?.vehicle_no || 'In-House Handover';
  const remarks = dydr.remarks || dydr.logistics?.remarks || '';

  const renderDeliveryCopy = (copyType, copyBadge) => (
    <div className="dydr-single-copy" style={{
      ...
      <div>
        <div style={{
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ 
              ...
              DYED YARN DELIVERY RECEIPT (DYDR)
            </div>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={metaValStyle}><strong>Date:</strong> {deliveryDate}</div>
            {deliveryTime && <div style={metaValStyle}><strong>Time:</strong> {deliveryTime}</div>}
          </div>
          <div>
            <div style={metaLabelStyle}>TARGET PROCESS & FORM</div>
            <div style={metaValStyle}><strong>Process:</strong> <span style={{ fontWeight: '700', color: '#7f1d1d' }}>{processLabel}</span></div>
            <div style={metaValStyle}><strong>Form No:</strong> {formNumber}</div>
            {machineName && machineName !== '—' && (
              <div style={metaValStyle}><strong>Machine:</strong> {machineName}</div>
            )}
          </div>
          <div>
            <div style={metaLabelStyle}>DESTINATION / PARTNER</div>
            <div style={metaValStyle}><strong>Type:</strong> {isJobWork ? 'Job Work Partner' : 'In-House Production'}</div>
            <div style={metaValStyle} title={partnerLabel}><strong>Unit/Partner:</strong> {partnerLabel}</div>
            {partnerDetails?.gstin && (
              <div style={metaValStyle}><strong>GSTIN:</strong> {partnerDetails.gstin}</div>
            )}
          </div>
          <div>
            <div style={metaLabelStyle}>LOGISTICS & HANDOVER</div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '0.4rem', fontSize: '0.72rem' }}>
          <thead>
            <tr style={{ backgroundColor: '#f1f5f9', borderTop: '1.5px solid #334155', borderBottom: '1.5px solid #334155' }}>
              ...
              <th style={{ ...compactThStyle, width: '8%' }}>From Loc</th>
          ...
          <tbody>
            {rawItems.length === 0 ? (
              ...
              rawItems.map((it, i) => {
                const orderNum = it.orderNo || it.order_no || it.orders?.order_number || it.order_number || dydr.order_no || dydr.orders?.order_number || '—';
                ...
                const yarnCount = formatCount(it);
                const colour = it.colour || it.color || '—';
                const yarnType = it.type || it.yarn_type || it.process_type || '—';
                const lotNo = it.lot_number || it.lot_no || '—';
                const locationName = typeof it.location === 'object' 
                  ? it.location?.location_name 
                  : (it.location || it.location_name || it.master_locations?.location_name || '—');
      ...
      <div style={{
        ...
        <div style={{ minWidth: '180px', textAlign: 'right' }}>
          <div style={{ fontWeight: '800', color: '#1a1a1a', marginBottom: '1.25rem', fontSize: '0.72rem' }}>
            For {partnerLabel !== 'In-House' ? partnerLabel : 'ASHOK TEXTILES'}
          </div>
  ...
  return (
    <div className="print-overlay" style={{
      ...
      >
        ...
        <div className="no-print" style={{ 
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <h2 style={{ margin: 0, fontSize: '1rem', fontWeight: '900', color: '#0f172a' }}>
              Dyed Yarn Delivery Note: {receiptNumber}
            </h2>
            <span style={{
              fontSize: '0.7rem',
        ...
        <div id="printable-dydr" className="printable-content" style={{ backgroundColor: '#fff' }}>
          ...
          {renderDeliveryCopy('duplicate', 'Duplicate / Production Copy')}
